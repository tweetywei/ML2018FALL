...
def get_eval_spec():
    ...
    """
    #image_patches_reshaped = tf.reshape(tf.extract_image_patches(image, [1,128,128,1], [1,128,128,1], [1, 1, 1, 1], 'VALID'), [-1, 128, 128, 3])
    #transform = lambda image: tf.reshape(tf.extract_image_patches(image, [1,128,128,1], [1,128,128,1], [1, 1, 1, 1], 'VALID'), [-1, 128, 128, 3])
    transform = lambda image: tf.image.resize_images(image, [224, 224])
    return {"transform": transform, "batchsize": 20}
...
def before_batch(train_history, validation_history):
    ...
    """
    global lr
    return {'feed_dict': {lr: 0.01}, 'batch_norm': 0.93, 'drop_out': None}
